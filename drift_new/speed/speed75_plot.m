per_data = xlsread('nonstop50_baregrey_2layer_heat_slp1_spe75_pull20_per.xlsx');
...
locs(id,:) = [];
pks(id,:) = [];
locs_1 = locs(12:2:end)-7;
pks_1 = pks(12:2:end);

id_t = find(loc_ts(:,1) < 5 | loc_ts(:,1) > 71);
loc_ts(id_t,:) = [];
troughs(id_t,:) = [];
loc_ts = loc_ts(12:2:end)-7;
troughs = troughs(12:2:end);

% subplot(212);
hold on
plot(time_per-7,per,'k');  
grid;
...
xlswrite('peaks_75.xls',matrix,'Sheet1');
xlswrite('troughs_75.xls',matrix_1,'Sheet1');