per_data = xlsread('nonstop50_bareblue_2layer_heat_slp1_spe25_pull20_per.xlsx');
...
locs(id,:) = [];
pks(id,:) = [];
locs_1 = locs(6:end)-4.6;
pks_1 = pks(6:end);

id_t = find(loc_ts(:,1) < 5 | loc_ts(:,1) > 72);
loc_ts(id_t,:) = [];
troughs(id_t,:) = [];
loc_ts = loc_ts(5:end)-4.6;
troughs = troughs(5:end);

time_per = time_per - 4.6;
% subplot(212);
hold on
plot(time_per,per,'k');  
grid;
...
xlswrite('nonstop50_bareblue_2layer_heat_slp1_spe25_pull20_per.csv',[time_per per],'Sheet1');
xlswrite('peaks_blue2.xls',matrix,'Sheet1');
xlswrite('troughs_blue2.xls',matrix_1,'Sheet1');